...
const DEFAUTS = [
  ...
  ['une exception est prise pour un démenti', 'outils/Controle.gs',
    "      etat: 'NON MESURÉ',",
    "      etat: 'DÉMENTI',"],
  ['une soumission qui n’obtient pas le verrou abandonne sans rien dire',
    'apps-script/Synchronisation.gs',
    'return limiteurSignalerLAbandon_(ligne);',
    'return { fait: false, abandon: true, ligne };'],
  ['la soumission n’attend plus le verrou', 'apps-script/Synchronisation.gs',
    'if (!verrou.tryLock(LIMITEUR_ATTENTE_VERROU_MS_)) {',
    'if (!verrou.tryLock(0)) {'],
  ['la marge ne se distingue plus d’un créneau vraiment plein',
